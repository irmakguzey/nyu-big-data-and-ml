...
import datasets
import numpy as np
from nanotron.config import DataArgs, DatasetStageArgs, PretrainDatasetsArgs
from nanotron.data.clm_collator import DataCollatorForCLM
from nanotron.data.dataloader import get_dataloader_worker_init, get_train_dataloader
...
def get_dataloader_from_data_stage(
    ...
    if data.dataset is None:
        log_rank(
            "Using custom data generator", logger=logger, level=logging.INFO, rank=0
        )
        ...
        #             (
        #                 trainer.global_batch_size * num_remaining_train_steps,
        #                 trainer.sequence_length + 1,
        #             ),
        #         ),
        #     }
        # )

        train_dataset, test_dset, tokenizer = get_datasets(
            root_dir="climate_text_dataset",
            preprocess=False,
            train_test_split=0.9,
            model_name=trainer.config.tokenizer.tokenizer_name_or_path,
            max_length=trainer.sequence_length + 1,
        )
        ###########################################################################################################
        ...
        train_dataloader = DataLoader(
            ...
            num_workers=0,
            pin_memory=True,
            worker_init_fn=get_dataloader_worker_init(
                dp_rank=trainer.parallel_context.dp_pg.rank()
            ),
        )

        # Wrap this dataloader to continue infinitely
...
def get_dataloader(trainer: DistributedTrainer) -> Dict[str, DataLoader]:
    ...
    for stage_idx, stage in enumerate(trainer.config.data_stages):
        ...
        consumed_train_samples, _ = get_consumed_train_samples_of_a_data_stage_from_ckp(
