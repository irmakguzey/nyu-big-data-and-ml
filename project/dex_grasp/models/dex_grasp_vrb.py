# Implementation of vrb model but edited for dex grasp
...
class VRBModel(nn.Module):
    ...
    def forward(
        self, feat, bbox_feat, valid_mask, future_hands, contact_point, future_valid
    ):
        B = feat.shape[0]
        fe, mu, cvar = self.encode_img(feat)
        gt_mu, gt_cvar = (
            contact_point[:, : self.n_maps],
            contact_point[:, self.n_maps :],
        )
        if self.var:
            obj_loss = self.mean_lmbda * torch.mean((mu - gt_mu) ** 2) + torch.mean(
                (cvar - gt_cvar) ** 2
            )
        else:
        ...
        # hand cvae
        pred_hand, traj_loss, traj_kl_loss = self.hand_head(
            memory[:, 0, :],
            target_hand,
            future_valid,
            contact_point=None,
