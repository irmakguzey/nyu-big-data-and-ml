...
class PosVAE(nn.Module):
    ...
    def __init__(
        ...
        self.pos_MLP = nn.Sequential(nn.Linear(in_dim, in_dim), nn.ELU())
        self.dec_MLP = nn.Sequential(
            nn.Linear(dec_dim, hidden_dim), nn.ELU(), nn.Linear(hidden_dim, in_dim)
        )
...
class TrajAffCVAE(nn.Module):
    def __init__(
        ...
        if self.condition_contact:
            ...
            self.contact_to_feature = nn.Sequential(
                nn.Linear(2, coord_dim, bias=False), nn.ELU(inplace=True)
            )
    ...
    def forward(
        self, context, target_hand, future_valid, contact_point=None, return_pred=False
    ):
        ...
        if self.condition_contact:
            ...
            contact_feat = einops.repeat(
                contact_feat, "m n -> m p q n", p=2, q=time_steps
            )
        ...
        if not return_pred:
            ...
            pred_hand, recon_loss, KLD = self.cvae(
                target_hand, c=condition_context, return_pred=return_pred
            )
    ...
    def inference(self, context, contact_point=None):
        ...
        z = (
            self.z_scale
            * torch.randn([context.shape[0], self.latent_dim], device=context.device)
            * 0
        )
