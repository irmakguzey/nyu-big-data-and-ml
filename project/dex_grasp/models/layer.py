...
from dex_grasp.models.net_utils import DropPath, get_pad_mask
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.attention = ScaledDotProductAttention(
            temperature=qk_scale or head_dim**0.5
        )
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, tgt, memory, memory_mask=None, trg_mask=None):
        ...
        tgt = tgt + self.drop_path(
            self.self_attn(q=tgt_2, k=tgt_2, v=tgt_2, mask=trg_mask)
        )
        tgt = tgt + self.drop_path(
            self.enc_dec_attn(q=self.norm2(tgt), k=memory, v=memory, mask=memory_mask)
        )
